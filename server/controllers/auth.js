...
exports.sendOTP = async (req, res) => {
  try {
    ...
    if (checkUserPresent) {
      console.log('(when otp generated) User already registered');
    ...
    // send otp in mail
    await mailSender(email, 'Verificación OTP', otpTemplate(otp, name));
...
exports.signup = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: error.message,
      messgae: "User coudn't be registered. Please try again..!",
    });
  }
};
  ...
  try {
    ...
    try {
      const emailResponse = await mailSender(
        ...
        passwordUpdated(
          updatedUserDetails.email,
          `Se ha actualizado la contraseña para ${updatedUserDetails.firstName} ${updatedUserDetails.lastName}`
        )
      );
